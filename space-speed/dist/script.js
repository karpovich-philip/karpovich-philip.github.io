...
const movePlayerShip = () => {
	const spaceShip = document.querySelector('.space_ship');
	const speed = 10;

	if (!spaceShip || __WEBPACK_IMPORTED_MODULE_0__common__["gameState"].gameOver) return;

	const coordinates = spaceShip.getBoundingClientRect();

	if (input.isDown('DOWN') || input.isDown('s')) {
		const edgeOfWindow = document.documentElement.clientHeight;
	...
	if (input.isDown('RIGHT') || input.isDown('d')) {
		const edgeOfWindow = document.documentElement.clientWidth;
...
const getTopPosOfEntity = (entity, storage) => {
	const spaceEntity = document.getElementsByClassName(entity);
...
const main = () => {
	const now = Date.now();
	const timePassed = (now - __WEBPACK_IMPORTED_MODULE_0__common__["gameState"].lastTime) / 1000;
...
const checkCollisionPlayerBulletWithEntity = () => {
	...
	for (let i = 0; i < __WEBPACK_IMPORTED_MODULE_0__common__["gameState"].storageOfPlayerBullets.length; i++) {
		const bullet = __WEBPACK_IMPORTED_MODULE_0__common__["gameState"].storageOfPlayerBullets[i].getBoundingClientRect();

		for (let n = 0; n < __WEBPACK_IMPORTED_MODULE_0__common__["gameState"].storageSpaceStuff.length; n++) {
			const entityData = __WEBPACK_IMPORTED_MODULE_0__common__["gameState"].storageSpaceStuff[n].getBoundingClientRect();
...
class SpaceEntity {
	...
	addEntity() {
		...
		newEntity.id = __WEBPACK_IMPORTED_MODULE_0__common__["gameState"].count;
